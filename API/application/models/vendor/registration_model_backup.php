...
class Registration_model extends CI_Model
{
        ...
        if ($totalcount > 0)
        {
			...
			switch ($data['sort_type']) {
             ...
             $this->db->select('
								...
								(SELECT COUNT(*) FROM SMNTP_MESSAGES WHERE RECIPIENT_ID = ' . $data['user_id'] . ' AND IS_READ = -1 AND INVITE_ID = MES.INVITE_ID) AS MESSAGE_COUNT
	...
	function get_ra_docs($data = array())
	{
		...
		if($data['vendor_type'] == 1){
			...
			$result = $this->db->query($query, array($data['trade_vendor_type'], $data['ownership']))->result_array();
			...
			if( ! empty($data['category_id'])){
				...
				$categories_str = (is_array($data['category_id']) ? implode(',', $data['category_id']) : $data['category_id']);
				$query = 'SELECT DISTINCT CATEGORY_ID FROM SMNTP_VP_REQUIRED_AGMT_DEFN 
						WHERE VENDOR_TYPE_ID = ? AND 
						OWNERSHIP_ID = ? AND 
						CATEGORY_ID IN (' . $categories_str .') ';
				$total_no_secondary_document = $this->db->query($query, array($data['vendor_type'], $data['ownership']))->result_array();
			...
			$query = 'SELECT DISTINCT A.VENDOR_TYPE_ID,
						...
						WHERE A.VENDOR_TYPE_ID = ?  AND A.OWNERSHIP_ID = ? AND A.ACTIVE = 1 AND B.ACTIVE = 1';
			if(!empty($data['category_id'])){
				$query .= ' AND ( CATEGORY_ID IN (' . implode(',', $data['category_id']) . ') ';
...
/*
	...
	function get_vendor_invite_details($data)
	{
		...
		$this->db->join('SMNTP_VENDOR_INVITE SVI_THREE', 'SVI.VENDOR_INVITE_ID = SVI_THREE.CC_VENDOR_CODE','LEFT');
	...
	function backup_table($vendor_code)
	{
		...
		$backup_smntp_vendor_owners = "INSERT INTO SMNTP_VENDOR_OWNERS_SYS_LOGS(VENDOR_OWNER_ID,VENDOR_ID,NAME,POSITION,DATE_CREATED,ACTIVE,FIRST_NAME,LAST_NAME,MIDDLE_NAME,BACKUP_DATE) SELECT SVCD.*, CURRENT_DATE FROM SMNTP_VENDOR_OWNERS SVCD WHERE VENDOR_ID = ".$vendor_code;
		$backup_smntp_vendor_rep = "INSERT INTO SMNTP_VENDOR_REP_SYS_LOGS(VENDOR_REP_ID,VENDOR_ID,NAME,POSITION,DATE_CREATED,ACTIVE,FIRST_NAME,LAST_NAME,MIDDLE_NAME,BACKUP_DATE) SELECT SVRSL.*, CURRENT_DATE FROM SMNTP_VENDOR_REP SVRSL WHERE VENDOR_ID = ".$vendor_code;
...
*/
	...
	function insert_logs($vendor_id, $user_id, $var_from, $var_to, $modified_field){
		...
		$insert_log = "INSERT INTO SMNTP_VENDOR_AUDIT_LOGS(VENDOR_ID, USER_ID, VAR_FROM, VAR_TO, MODIFIED_FIELD, MODIFIED_DATE) VALUES(".$vendor_id.",".$user_id.",'".$var_from."','".$var_to."','".$modified_field."','".$date_modified."')";
