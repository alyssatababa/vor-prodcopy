...
class mms_report extends CI_Model
{
	...
	function get_data(){
		...
		$date_to = $this->db->query("SELECT * FROM SMNTP_VENDOR_STATUS_LOGS WHERE status_id = 19 AND DATE_UPDATED BETWEEN DATE_SUB(".$date.", INTERVAL ".$sys." DAY) AND ".$date." -- AND VENDOR_INVITE_ID IN(22849)");
		//$date_to = $this->db->query("SELECT * FROM SMNTP_VENDOR_STATUS_LOGS WHERE status_id = 19 AND DATE_UPDATED BETWEEN DATE_SUB(".$date.", INTERVAL ".$sys." DAY) AND ".$date." AND VENDOR_INVITE_ID IN(22590)");
			...
			for($e=0; $e < $count_date_to; $e++){
				...
				if($count_date_from > 0){
					...
					$get_vendor_invite_id = $this->db->query("SELECT * FROM SMNTP_VENDOR_STATUS_LOGS 
						...
						AND STATUS_ID = 250
						AND VENDOR_INVITE_ID = ".$result_date_to[$e]->VENDOR_INVITE_ID." ORDER BY DATE_UPDATED LIMIT 0,1");
