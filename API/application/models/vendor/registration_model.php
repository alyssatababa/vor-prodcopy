...
*/
	...
	function check_additional_requirements_upload($vendor_id)
	{
		...
		if( ! empty($result_vendor)){
			...
			$avc_categories = $this->db->query('SELECT CATEGORY_ID FROM SMNTP_VENDOR_AVC_CAT WHERE VENDOR_INVITE_ID = ?', array($vendor_invite_id))->result_array();
			...
			if($vendor_type == 1 || $vendor_type == 3){
				...
				foreach($avc_categories as $avc_category){
					if( ! empty($avc_category['CATEGORY_ID']) && ! in_array($avc_category['CATEGORY_ID'],$cat_array)){
						$cat_array[] = $avc_category['CATEGORY_ID'];
					}
				}
	...
	function get_ra_docs3($data){
		...
		$avc_categories = $this->db->query('SELECT CATEGORY_ID FROM SMNTP_VENDOR_AVC_CAT WHERE VENDOR_INVITE_ID = ?', array($data['invite_id']))->result_array();
