...
class Invitecreation_model extends CI_Model
{
	...
	{
		$this->db->distinct();
		$this->db->select('SCat.CATEGORY_NAME, SSCat.SUB_CATEGORY_NAME');
		$this->db->from('SMNTP_VENDOR_CATEGORIES SVC');
	...
	{		
		...
		$this->db->select('SVI.TRADE_VENDOR_TYPE, SVI.VENDOR_NAME, SVI.CONTACT_PERSON,
							...
							SSCA.SUB_CATEGORY_NAME, SVS.TERMSPAYMENT, SVS.AVC_TERMSPAYMENT, SVAI.DATE_CREATED, SV.VENDOR_ID, SV.VENDOR_CODE, SV.VENDOR_TYPE, SVI.VENDOR_INVITE_ID, SV.VENDOR_CODE_02,
							SCA_TWO.CATEGORY_NAME AS AVC_CATEGORY_NAME, SSCA_TWO.SUB_CATEGORY_NAME AS AVC_SUB_CATEGORY_NAME, SVI.REGISTRATION_TYPE, SVI.PREV_REGISTRATION_TYPE');
