...
/*
		...
		function update_user($data,$vrd,$vrdh)
		{
			...
			if($utype == 2){
				...
				$vendor_invite_id = $this->db->query('SELECT SVI.VENDOR_INVITE_ID, SV.VENDOR_ID FROM SMNTP_VENDOR_INVITE SVI LEFT JOIN SMNTP_VENDOR SV ON SVI.VENDOR_INVITE_ID = SV.VENDOR_INVITE_ID WHERE SVI.USER_ID = ?', $uid)->row_array();
				
				if($vendor_invite_id['VENDOR_ID'] != ''){
					$info3 = array('VENDOR_NAME' => $data['USER_FIRST_NAME']);
					$where_clause = array('VENDOR_INVITE_ID' => $vendor_invite_id['VENDOR_INVITE_ID']);
					$this->db->where($where_clause);
					$this->db->update('SMNTP_VENDOR',$info3);
				}
