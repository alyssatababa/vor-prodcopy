...
class Common_model extends CI_Model
{
   ...
   function reset_password($data)
   {
		...
		$user_info_two = $this->db->query('SELECT USER_TYPE_ID FROM SMNTP_USERS WHERE USER_ID = ?', $user_info['USER_ID'])->row_array();
		$user_type = $user_info_two['USER_TYPE_ID'];
   ...
   function get_email_template($data)
	...
	{
		$this->db->select('U.USER_ID, C.USERNAME, U.USER_EMAIL, (U.USER_FIRST_NAME || \' \' || U.USER_MIDDLE_NAME || \' \' || U.USER_LAST_NAME) AS USERS_NAME, C.DEACTIVATED_FLAG, U.USER_STATUS');
		...
		//$this->db->where('U.USER_STATUS_ID !=', '0');
		...
		if ($rows > 0) {
			if($rs->row()->DEACTIVATED_FLAG != 1){
				if($rs->row()->USER_STATUS != 0){
					$err_code = '1';
				}else{
					$err_code = '3';
				}
	...
	function active_tokens()
	{
		$this->db->select('SVT.*');
		$this->db->from('SMNTP_VENDOR_TOKEN SVT');
		$this->db->join('SMNTP_VENDOR_INVITE SVI','SVI.VENDOR_INVITE_ID = SVT.VENDOR_INVITE_ID');
		$this->db->where('SVT.ACTIVE', 1);
		$this->db->where('SVI.REGISTRATION_TYPE', 1);
