<?Php 	defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Menus extends REST_Controller {
		...
		public function menu_by_name_get(){
			...
			$data = $this->menus_model->read_like_name($menu_name);
			// var_dump($data);
			if ($data)
			{
				$this->response($data);
		...
		public function menu_put(){
			...
			$data = $this->menus_model->read_by_name($menu_name);
			...
			if ($menu_id && ($menu_name!=null && $menu_name!='')) {
				if ($data && isset($data->menu_id)) {
					if ($data->menu_id == $menu_id) {
						// do insert
						$this->menus_model->insert_menu($menu_name,$description,$sorting,$menu_link);
						$data = $this->menus_model->read_by_name($menu_name);
		...
		public function menu_delete(){
			...
			$data = $this->menus_model->read($menu_id);
