<?Php 	defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Rfq_bid_monitor_approval extends REST_Controller {
		...
		public function view_rfqb_apprvl_post(){
			$rfb_id = $this->post('rfb_id');
			$data = $this->rfq_bid_monitor_model->select_rfqrfb_approval($rfb_id);
			if ($data)
			{
				$this->response($data);
			}
		...
		// public function view_rfqb_apprvl_position_post(){
		// 	$rfb_id = $this->post('rfb_id');
		...
		public function set_rfqb_status_post(){
			...
			$invite_status = $this->post('invite_status');
			$reason = $this->post('reason');
			$position_id = $this->post('position_id');
			$extension_date = $this->post('extension_date');
			$current_status = $this->post('current_status');
			$data = $this->rfq_bid_monitor_model->set_rfqrfb_status($rfb_id, $rfb_status, $reason, $position_id, $extension_date, $invite_status, $user_id, $current_status);
			if ($data)
			{
				$this->response($data);
			}
