...
class Registration_api extends REST_Controller
{
	...
	function registration_add_post()
	{
		...
		if ($real_status_id > 9 && $real_status_id != 11)
		{
			...
			$get_vendor_request = $this->registration_model->al_vendor_request($var['invite_id']);
				$vendor_request_row_count = count($get_vendor_request);
				$result = '';
				if($vendor_request_row_count > 0){
					for($a=0; $a<$vendor_request_row_count; $a++){

						//if($get_vendor_request[$a]['VENDOR_PASS_QTY'] != 0){

							/*$insert_vrpass_record = $this->registration_model->insert_vendor_request_logs($var['invite_id'], $get_vendor_request[$a]['DATE_CREATED'],  $get_vendor_request[$a]['REQUEST_TYPE'], $get_vendor_request[$a]['VENDOR_PASS_QTY']);*/

							$insert_vrpass = $this->registration_model->insert_vrpass($get_vendor_request[$a]['VENDOR_REQUEST_ID'], $get_vendor_request[$a]['VENDOR_INVITE_ID'], $get_vendor_request[$a]['DATE_CREATED'], $get_vendor_request[$a]['APPROVAL_DATE'], $get_vendor_request[$a]['VENDOR_NAME'], $get_vendor_request[$a]['TOTAL_PASS_QTY'], $get_vendor_request[$a]['TOTAL_AMOUNT_DEDUCTION'], $get_vendor_request[$a]['USER_ID']);
					...
					//print_r($insert_vrpass);return;
				}

				$get_vendor_id_request = $this->registration_model->al_vendor_id_request($var['invite_id']);
					$vendor_id_request_row_count = count($get_vendor_id_request);
					$result = '';
					if($vendor_id_request_row_count > 0){
						for($c=0; $c<$vendor_id_request_row_count; $c++){
									/*$insert_record = $this->registration_model->insert_vendor_id_request_logs($var['invite_id'], $get_vendor_id_request[$c]['DATE_CREATED'],  $get_vendor_id_request[$c]['REQUEST_TYPE'], $get_vendor_id_request[$c]['LAST_NAME'], $get_vendor_id_request[$c]['MIDDLE_INITIAL'], $get_vendor_id_request[$c]['FIRST_NAME'], $get_vendor_id_request[$c]['DESIGNATION']);*/

									
									$insert_vendor_id_request = $this->registration_model->insert_vrid($get_vendor_id_request[$c]['VENDOR_REQUEST_ID'],  $get_vendor_id_request[$c]['TRADE_VENDOR_TYPE'], $get_vendor_id_request[$c]['FIRST_NAME'], $get_vendor_id_request[$c]['MIDDLE_INITIAL'], $get_vendor_id_request[$c]['LAST_NAME'], $get_vendor_id_request[$c]['DESIGNATION'], $get_vendor_id_request[$c]['REQUEST_TYPE']);

						}

						//print_r($get_vendor_id_request);return;
					}
		...
		else
		{
			...
			if($add_audit_logs == true){
				 ...
				 $get_vendor_request = $this->registration_model->al_vendor_request($var['invite_id']);
                    ...
                    if($vendor_request_row_count > 0){
                            for($a=0; $a<$vendor_request_row_count; $a++){
                            	...
                            	//if($get_vendor_request[$a]['VENDOR_PASS_QTY'] != 0){
	                                /*$insert_vrpass_record = $this->registration_model->insert_vendor_request_logs($var['invite_id'], $get_vendor_request[$b]['DATE_CREATED'],  $get_vendor_request[$b]['REQUEST_TYPE'], $get_vendor_request[$b]['VENDOR_PASS_QTY']);*/
	                                $insert_vrpass = $this->registration_model->insert_vrpass($get_vendor_request[$a]['VENDOR_REQUEST_ID'], $get_vendor_request[$a]['VENDOR_INVITE_ID'], $get_vendor_request[$a]['DATE_CREATED'], $get_vendor_request[$a]['APPROVAL_DATE'], $get_vendor_request[$a]['VENDOR_NAME'], $get_vendor_request[$a]['TOTAL_PASS_QTY'], $get_vendor_request[$a]['TOTAL_AMOUNT_DEDUCTION'], $get_vendor_request[$a]['USER_ID']);
                            	//}
                                
                            }
                            //print_r('here1');return;
                            //print_r($insert_vrpass_record);return;
                            //var_dump($insert_vrpass);die;
                    }
                    $get_vendor_id_request = $this->registration_model->al_vendor_id_request($var['invite_id']);
                    $vendor_id_request_row_count = count($get_vendor_id_request);
				...
				$count_to_insert = $this->post('opd_count');
                                $get_vendor_request = $this->registration_model->al_vendor_request_two($var['invite_id']);
                                $vendor_request_row_count = count($get_vendor_request);
                                $result = '';
                                if($vendor_request_row_count > 0){
                                        for($b=0; $b<$vendor_request_row_count; $b++){
	                                        	//if($get_vendor_request[$b]['VENDOR_PASS_QTY'] != 0){
	                                            /*$insert_record = $this->registration_model->insert_vendor_request_logs($var['invite_id'], $get_vendor_request[$b]['DATE_CREATED'],  $get_vendor_request[$b]['REQUEST_TYPE'], $get_vendor_request[$b]['VENDOR_PASS_QTY']);*/
	                                            $insert_vrpass = $this->registration_model->insert_vrpass($get_vendor_request[$b]['VENDOR_REQUEST_ID'], $get_vendor_request[$b]['VENDOR_INVITE_ID'], $get_vendor_request[$b]['DATE_CREATED'], $get_vendor_request[$b]['APPROVAL_DATE'], $get_vendor_request[$b]['VENDOR_NAME'], $get_vendor_request[$b]['TOTAL_PASS_QTY'], $get_vendor_request[$b]['TOTAL_AMOUNT_DEDUCTION'], $get_vendor_request[$b]['USER_ID']);
                                        	//}
                    
                                            //print_r($insert_vendor_request);return;
                                        }
                                        //print_r('here3');return;
                                }
                                //var_dump($get_vendor_request);die;
                                $get_vendor_id_request = $this->registration_model->al_vendor_id_request_two($var['invite_id']);
                                $vendor_id_request_row_count = count($get_vendor_id_request);
                                $result = '';
                                if($vendor_id_request_row_count > 0){
                                        for($c=0; $c<$vendor_id_request_row_count; $c++){
                                        ...
                                        //print_r('here4');return;
                                }
                                
                                //var_dump($get_vendor_id_request);die;
			}
	...
	function save_uploads($var)
	{
		...
		$avc_category_sup_count = $this->post('avc_cat_sup_count');
		...
		$avc_cat_array = '';
                for($i = 1; $i <= $avc_category_sup_count; $i++){
                        if(!($categoryid = $this->post('category_id' . $i))) {
                                continue;
                        }
                        $avc_cat_array .= $categoryid . ',';
                        $avc_cat_array .= $this->post('category_id' . $i) . ',';
                }
		
		$docs_var['category_id'] = explode(',', rtrim($cat_array, ','));
		if($avc_cat_array){
        	$docs_var['category_id'] = array_merge(explode(',', rtrim($avc_cat_array, ',')), $docs_var['category_id']);
        }
	...
	function vendor_data_get()
	{
		...
		$rs_cat_exception = $this->registration_model->get_category_exception();
		...
		$data['rs_cat_exception']		= $rs_cat_exception->result_array();
	...
	function vendor_info_get()
	{
		...
		//$where_arr = array('VENDOR_INVITE_ID' => $invite_id);
		$vendor_code = $this->common_model->get_from_table_where_array('SMNTP_VENDOR', 'VENDOR_CODE', $where_arr);
		$cc_vendor_code = $this->common_model->get_from_table_where_array('SMNTP_VENDOR_INVITE', 'CC_VENDOR_CODE', $where_arr);
		$where_arr_cc = array('VENDOR_INVITE_ID' => $cc_vendor_code);
		$current_vt = $this->common_model->get_from_table_where_array('SMNTP_VENDOR_INVITE', 'TRADE_VENDOR_TYPE', $where_arr);
		$prev_vt = $this->common_model->get_from_table_where_array('SMNTP_VENDOR', 'TRADE_VENDOR_TYPE', $where_arr_cc);
