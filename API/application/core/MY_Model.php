...
class MY_Model extends CI_Model
{
     ...
     */
    public function update($primary_value, $data, $escape = TRUE,$skip_validation = FALSE)
    {
        ...
        if ($data !== FALSE)
        {
            $result = $this->_database->where((is_array($primary_value) ? $primary_value : $this->primary_key), $primary_value)
                               ->set($data,$escape)
                               ->update($this->_table);
    ...
    public function orders_custom($criteria, $order = 'ASC',$limit){
        if ( is_array($criteria) )
        {
            foreach ($criteria as $key => $value)
            {
               $result =  $this->_database->order_by($key, $value)->limit($limit);
            }
        }
        else
        {
            $result = $this->_database->order_by($criteria, $order)->limit($limit);
        }
        return $result;
    }
