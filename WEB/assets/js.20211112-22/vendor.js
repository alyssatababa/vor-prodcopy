...
function check_existing_record()
{
    ...
    var post_params = "vendorname="+encodeURIComponent($('#txt_vendorname').val().trim()) + "&invite_id=" + $('#invite_id').val();
...
function check_status()
{
    ...
    if ($('#status_id').val() > 9 && $('#status_id').val() != 11) // ($('#status_id').val() == 190 || $('#status_id').val() == 195) // aditional req disable all exclude #additional req //190 = additional req, 195 = incomplete additional req
    {
...
$(document).ready(function() {
   ...
   //####################################################### Vendor Registration Invite  START
    ...
    $('table').on('click', '.reviewed, .validated, .na_reviewed_additional', function(){
        ...
        $(this).closest('td').next().find(':input').val(sysdate);
		var sid_temp = parseInt($("#status_id").val());
...
/*        console.log(x);
        ...
        get_list_docs(ownership, trade_vendor_type,cat_sup, vendor_type);
