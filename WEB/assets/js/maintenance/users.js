...
$(document).on("click", ".view_pend", function (e) {   
	   e.preventDefault();
		if (e.handled !== true) { //Checking for the event whether it has occurred or not.
			clear_alert();
			let uid = this.getAttribute('data-ui');
			document.getElementById("tbl_pending_record").tBodies[0].innerHTML = "";
			get_view_pending_records(uid);
		}
		e.stopImmediatePropagation();
});

function get_view_pending_records(uid){;
	$.ajax({
		type:'POST',
		data:{uid: uid},
		url:'maintenance/users/view_pending',		
		success: function(result){		
			create_pending_table(result);
		}
	});
}

function create_pending_table(data)
{
	data = JSON.parse(data);
	pending_rec = "";
	
	let table = document.getElementById('tbl_pending_record').tBodies[0];
	if(data.length > 0){
		for( a = 1; a<data.length; a++){
			pending_rec += "<tr><td>"+data[a].VENDOR_NAME+"</td><td>"+data[a].STATUS_NAME+"</td></tr>"
		}
	}else{
		pending_rec += "<tr><td colspan='2'>No Record Found.</td></tr>"
	}
	table.innerHTML = pending_rec;
}
...
function get_edit_data(uid)
{
	...
	$.ajax({
		...
		uival_change('user_status_edit',n['INFORMATION'][0]['USER_STATUS']);
...
_data = {
		...
		em: $('#email_edit').val(),
		pw: $('#pass_edit').val(),
		user_status: $('#user_status_edit').val(),
		cpw: $('#confirm_pass_edit').val(),
		pos: $('#slt_user_position_edit option:selected').data('pos')
	}

	//console.log(_data);
	//console.log(_data['pos']);
	if(_data['pos'] == 10){
		$("#ln_edit").attr("data-exclude","exc");
		$("#mn_edit").attr("data-exclude","exc");
		$("#mo_edit").attr("data-exclude","exc");
	}else{
		$("#ln_edit").removeAttr("data-exclude");
		$("#mn_edit").removeAttr("data-exclude");
		$("#mo_edit").removeAttr("data-exclude");
	}
...
$(document).on('click', '#btn_vw_user_status_history', function(e){
	e.preventDefault();
	$.ajax({
		type:'GET',
		data:{uid: $('#hel').val()},
		url:'maintenance/users/get_user_status_history_body/',
		success: function(result){
			$('#vw_user_status_history #tbl_user_status_history_body').html(result);
		}	
	}).fail(function(){
