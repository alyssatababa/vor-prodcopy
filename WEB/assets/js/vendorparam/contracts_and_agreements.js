...
$(document).on("click", "#btn_save_ca",function(e)
{
    ...
    if (e.handled !== true) { //Checking for the event whether it has occurred or not.
		...
		var tool_tip = $("#edit_tool_tip").val();
		...
		if(isFillUpError(ca_type) == 0){
			if (ca_type == "1"){
				loading('btn_save_ca','in_progress');
				addCA(ca_name, description, bus_division, downloadable, viewable, tool_tip);	
			}else if (ca_type == "2"){
				loading('btn_save_ca','in_progress');
				var ca_id = $("#edit_ca_id").val();
				saveCA(ca_id, ca_name, description, bus_division, downloadable, viewable, tool_tip)
			}	
		}
	}
	e.stopImmediatePropagation();
...
function createDataTable(obj_result)
{
	...
	for(x=0;x<z;x++){
		...
		if(obj_result[x].TOOL_TIP == null){
			tool_tip = '';
		}else{
			tool_tip = obj_result[x].TOOL_TIP;
		}
		
		//$('<tr id="row'+y+'" class="clickable-row"><td>'+y+'</td><td id="ca_name'+y+'">'+obj_result[x].REQUIRED_AGREEMENT_NAME+'</td><td id="description'+y+'" >'+obj_result[x].DESCRIPTION+'</td><td id="tool_tip'+y+'" >'+tool_tip+'</td><td id="bus_division'+y+'">'+obj_result[x].BUS_DIVISION+'</td><td id="ca_date'+y+'">'+ '<span style="display:none;">' + obj_result[x].DATE_SORTING_FORMAT +'</span>' + obj_result[x].CA_DATE+'</td><td><button id="edit'+y+'" data-id="row-'+y+'" class = "btn btn-default btn_tbl_upload" rel = "'+obj_result[x].REQUIRED_AGREEMENT_ID+'|'+obj_result[x].REQUIRED_AGREEMENT_NAME+'|'+obj_result[x].TOOL_TIP+'" href = "" onclick = "return false;" ><span class= "glyphicon glyphicon-sunglasses g_icon" onclick = "return false"></span></button></td><td><input type="checkbox" ' + d_checked + 'disabled></td><td><input type="checkbox" ' + v_checked + ' disabled></td><td><button class = "btn btn-default btn_tbl_edit" rel = "'+obj_result[x].REQUIRED_AGREEMENT_ID+'|'+obj_result[x].REQUIRED_AGREEMENT_NAME+'|'+obj_result[x].DESCRIPTION+'|'+tool_tip+'|'+obj_result[x].BUS_DIVISION +'|'+obj_result[x].DOWNLOADABLE +'|'+obj_result[x].VIEWABLE +'" href = "" onclick = "return false;"><span class= "glyphicon glyphicon-edit g_icon" onclick = "return false"></span></button> <span>  </span> <button rel = "'+obj_result[x].REQUIRED_AGREEMENT_ID+'" href = "" onclick = "return false;" class = "btn btn-default btn_tbl_delete"><span class="glyphicon glyphicon-trash gl-black" aria-hidden="true"></span></button></td></tr>').appendTo('#tbl_body');
		$('<tr id="row'+y+'" class="clickable-row"><td>'+y+'</td><td id="ca_name'+y+'">'+obj_result[x].REQUIRED_AGREEMENT_NAME+'</td><td id="tool_tip'+y+'" >'+tool_tip+'</td><td id="ca_date'+y+'">'+ '<span style="display:none;">' + obj_result[x].DATE_SORTING_FORMAT +'</span>' + obj_result[x].CA_DATE+'</td><td><button id="edit'+y+'" data-id="row-'+y+'" class = "btn btn-default btn_tbl_upload" rel = "'+obj_result[x].REQUIRED_AGREEMENT_ID+'|'+obj_result[x].REQUIRED_AGREEMENT_NAME+'|'+obj_result[x].TOOL_TIP+'" href = "" onclick = "return false;" ><span class= "glyphicon glyphicon-sunglasses g_icon" onclick = "return false"></span></button></td><td><input type="checkbox" ' + d_checked + 'disabled></td><td><input type="checkbox" ' + v_checked + ' disabled></td><td><button class = "btn btn-default btn_tbl_edit" rel = "'+obj_result[x].REQUIRED_AGREEMENT_ID+'|'+obj_result[x].REQUIRED_AGREEMENT_NAME+'|'+obj_result[x].DESCRIPTION+'|'+tool_tip+'|'+obj_result[x].BUS_DIVISION +'|'+obj_result[x].DOWNLOADABLE +'|'+obj_result[x].VIEWABLE +'" href = "" onclick = "return false;"><span class= "glyphicon glyphicon-edit g_icon" onclick = "return false"></span></button> <span>  </span> <button rel = "'+obj_result[x].REQUIRED_AGREEMENT_ID+'" href = "" onclick = "return false;" class = "btn btn-default btn_tbl_delete"><span class="glyphicon glyphicon-trash gl-black" aria-hidden="true"></span></button></td></tr>').appendTo('#tbl_body');
	}	
}
...
function addCA(ca_name, description, bus_division, downloadable, viewable, tool_tip)
{
	var base_url = $('#b_url').attr('data-base-url');

	$.ajax({
		type:'POST',
		data:{ca_name: ca_name, description: description, bus_division: bus_division, downloadable:downloadable, viewable:viewable, tool_tip:tool_tip},
		url: base_url + "add_ca",
		success: function(result){	
			//alert(result);
			var json_result = $.parseJSON(result);
...
function saveCA(ca_id, ca_name, description, bus_division, downloadable, viewable, tool_tip)
{
	var base_url = $('#b_url').attr('data-base-url');
	//alert(downloadable);
	//alert(viewable);
	$.ajax({
		type:'POST',
		data:{ca_id: ca_id, ca_name: ca_name, description: description, bus_division: bus_division, downloadable: downloadable, viewable:viewable, tool_tip:tool_tip},
