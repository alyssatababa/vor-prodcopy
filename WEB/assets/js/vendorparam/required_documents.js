    ...
    if (e.handled !== true) { //Checking for the event whether it has occurred or not.
		...
		if(isFillUpError(reqdocs_type) == 0){
			if (reqdocs_type == "1"){
				...
				addReqDocs(reqdocs_name, description, bus_division, tool_tip);	
			}else if (reqdocs_type == "2"){
				loading('btn_save_reqdocs','in_progress');
				var reqdocs_id = $("#edit_reqdocs_id").val();
				saveReqDocs(reqdocs_id, reqdocs_name, description, bus_division, tool_tip)
			}	
		}
	}
...
function createDataTable(obj_result)
{
	...
	for(x=0;x<z;x++){
		...
		// $('<tr id="row'+y+'" class="clickable-row"><td>'+y+'</td><td id="reqdocs_name'+y+'">'+obj_result[x].REQUIRED_DOCUMENT_NAME+'</td><td id="description'+y+'" >'+obj_result[x].DESCRIPTION+'</td><td id="tool_tip'+y+'" >'+tool_tip+'</td><td id="bus_division'+y+'">'+obj_result[x].BUS_DIVISION+'</td><td id="reqdocs_date'+y+'">'+ '<span style="display:none;">' + obj_result[x].DATE_SORTING_FORMAT +'</span>'+obj_result[x].REQDOCS_DATE+'</td><td><button id="edit'+y+'" data-id="row-'+y+'" class = "btn btn-default btn_reqdocs_upload" rel = "'+obj_result[x].REQUIRED_DOCUMENT_ID+'|'+obj_result[x].REQUIRED_DOCUMENT_NAME+'" href = "" onclick = "return false;" ><span class= "glyphicon glyphicon-sunglasses g_icon" onclick = "return false"></span></button></td><td><button class = "btn btn-default btn_reqdocs_edit" rel = "'+obj_result[x].REQUIRED_DOCUMENT_ID+'|'+obj_result[x].REQUIRED_DOCUMENT_NAME+'|'+obj_result[x].DESCRIPTION+'|'+tool_tip+'|'+obj_result[x].BUS_DIVISION+'" href = "" onclick = "return false;"><span class= "glyphicon glyphicon-edit g_icon" onclick = "return false"></span></button> <span>  </span> <button rel = "'+obj_result[x].REQUIRED_DOCUMENT_ID+'" href = "" onclick = "return false;" class = "btn btn-default btn_reqdocs_delete"><span class="glyphicon glyphicon-trash gl-black" aria-hidden="true"></span></button></td></tr>').appendTo('#tbl_body');
		$('<tr id="row'+y+'" class="clickable-row"><td>'+y+'</td><td id="reqdocs_name'+y+'">'+obj_result[x].REQUIRED_DOCUMENT_NAME+'</td><td id="tool_tip'+y+'" >'+tool_tip+'</td><td id="reqdocs_date'+y+'">'+ '<span style="display:none;">' + obj_result[x].DATE_SORTING_FORMAT +'</span>'+obj_result[x].REQDOCS_DATE+'</td><td><button id="edit'+y+'" data-id="row-'+y+'" class = "btn btn-default btn_reqdocs_upload" rel = "'+obj_result[x].REQUIRED_DOCUMENT_ID+'|'+obj_result[x].REQUIRED_DOCUMENT_NAME+'" href = "" onclick = "return false;" ><span class= "glyphicon glyphicon-sunglasses g_icon" onclick = "return false"></span></button></td><td><button class = "btn btn-default btn_reqdocs_edit" rel = "'+obj_result[x].REQUIRED_DOCUMENT_ID+'|'+obj_result[x].REQUIRED_DOCUMENT_NAME+'|'+obj_result[x].DESCRIPTION+'|'+tool_tip+'|'+obj_result[x].BUS_DIVISION+'" href = "" onclick = "return false;"><span class= "glyphicon glyphicon-edit g_icon" onclick = "return false"></span></button> <span>  </span> <button rel = "'+obj_result[x].REQUIRED_DOCUMENT_ID+'" href = "" onclick = "return false;" class = "btn btn-default btn_reqdocs_delete"><span class="glyphicon glyphicon-trash gl-black" aria-hidden="true"></span></button></td></tr>').appendTo('#tbl_body');
	}	
}
...
$(document).on("click", ".btn_reqdocs_edit", function (e) {
    ...
    if (e.handled !== true) { //Checking for the event whether it has occurred or not.
		...
		$('#edit_tool_tip').val(reqdocs[3]);
		if (reqdocs[4] == "TRADE"){
			$('#edit_bus_division').val("1");
		}else if (reqdocs[4] == "NON-TRADE"){
...
function addReqDocs(reqdocs_name, description, bus_division, tool_tip)
{
	var base_url = $('#b_url').attr('data-base-url');
...
/*	console.log(reqdocs_name + '/' + description + '/' + bus_division);
	...
	$.ajax({
		type:'POST',
		data:{reqdocs_name: reqdocs_name, description: description, bus_division: bus_division, tool_tip:tool_tip},
...
function saveReqDocs(reqdocs_id, reqdocs_name, description, bus_division, tool_tip)
{
	var base_url = $('#b_url').attr('data-base-url');
	
	$.ajax({
		type:'POST',
		data:{reqdocs_id: reqdocs_id, reqdocs_name: reqdocs_name, description: description, bus_division: bus_division, tool_tip:tool_tip},
