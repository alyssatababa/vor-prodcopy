...
class Setpassword extends CI_Controller
{
	...
	function index($token = null)
	{
		...
		{
			$data['token'] = $token;
			$rs = $this->rest_app->get('index.php/common_api/user_token_info', $data, 'application/json');
			//$this->rest_app->debug();
			$rs = (array)$rs;
			...
			if(!empty($rs)){
				...
				if ($resultcount > 0)
				{
					...
					if($today >= $expiry)
					{
						$message = '<strong>Set Account Password Expired!</strong>';
				...
				{
					$message = '<strong>Set Account Password Link Expired!</strong>';
					$alert_class = 'alert-danger';
		...
		$this->load->view('common/set_password', $data);
	}
	...
	function validate_password($token = null)
	{
		...
		if ($token != null)
		{
			...
			if ($resultcount > 0)
			{
				...
				$test = $rs;
				$data['username'] = $username;
				$rs = $this->rest_app->put('index.php/common_api/set_password', $data, 'text');
				...
				if (isset($rs->status) && $rs->status)
				{
					...
					$gmd = $this->rest_app->get('index.php/mail/get_email_template', array('template_id' => 45), 'application/json');
					$email_data['to'] = $rs['email'];
					$email_data['subject'] = "Successfully Created Account Password";
					$email_data['content'] = $gmd->CONTENT;
					$send_email = $this->rest_app->post('index.php/common_api/send_email_message', $email_data, '');
				}
