...
class Approval extends CI_Controller
{
	function index($id = 0)
	{
		...
		if ($all_data['num_rows'] > 0)
		{
			...
			$data['createdbyid']				= $all_data['result'][0]->CREATED_BY;
		...
		$this->load->view('rfqb/approval', $data);
	}
	...
	function response_creation_approval()
	{
		...
		$data['title_txt']			= $this->input->post('title');
		$data['createdbyid'] 		= $this->input->post('createdbyid');
		...
		// $this->rest_app->debug();
			$new_data['user_id'] 	= $data['createdbyid'];
			$buyer_data 			= (array)$this->rest_app->get('index.php/rfq_api/user_data/', $new_data, 'application/json');

			$data_email['template_id'] = 42;
			$data_email['type_id'] = 2; 
			$data_email['status'] = $data['status']; // on dev config  = 61  
			$message_data	 	= (array)$this->rest_app->get('index.php/rfq_api/mail_to_approver/', $data_email, 'application/json');
		
			$title = 'RFQ#'.$data['rfx_id'].' - '.$data['title_txt'];

			$msg = str_replace('[approvername]', $this->session->userdata('user_first_name') .' '.$this->session->userdata('user_last_name'), $message_data['message']);
			$msg = str_replace('[rfqtitle]', $title, $msg);
			$msg = str_replace('[requestor]', $buyer_data['result'][0]->USER_FIRST_NAME.' '.$buyer_data['result'][0]->USER_LAST_NAME, $msg);
		}
		else
		{
			$data['nxt_position_id']	= $next_status_data['result'][0]->REJECT_POSITION_ID; // 22=published or approved
			...
			$msg = str_replace('[buyername]', $buyer_data['result'][0]->USER_FIRST_NAME.' '.$buyer_data['result'][0]->USER_LAST_NAME, $msg);
			$msg = str_replace('[rejectreason]', $data['reject_reason'], $msg);
		...
		#email for vendors invited
		$new_data['id'] = $data['rfx_id'];
		$invited_vendors_array	 			= $this->rest_app->get('index.php/rfq_api/get_invite_vendors/', $new_data, 'application/json');

		$data_email['template_id'] = 0;
		$data_email['type_id'] = 2; 
		$data_email['status'] = 998;  
		$message_data	 	= $this->rest_app->get('index.php/rfq_api/mail_to_approver/', $data_email, 'application/json');
		...
		if($invited_vendors_array->result_count > 0)
		{
			...
			foreach($invited_vendors_array->result as $row)
			{
				$msg = str_replace('[vendorname]', $row->VENDOR_NAME, $message_data->message);
				$msg = str_replace('[rfqtitle]', $title, $msg);
	...
	function rfq_approval_table($rfqrfb_id = null)
	{
		$data['user_id'] 		= $this->session->userdata('user_id');
