<?php
Class Rfq_Response_Creation extends CI_Controller{
	...
	function index($id = 0, $invite_id)
	{
		...
		$quote_list = '';
		$all_data 					= (array)$this->rest_app->get('index.php/rfq_api/load_approval_data/', $data, 'application/json');
		$data['table'] = '';
		//$this->rest_app->debug();
				        		...
				        		$old_image = '#';
									...
									$quote_list .= '<input type="hidden" name="counter_offer_hidden'.$x.'_'.$p.'" id="counter_offer_hidden'.$x.'_'.$p.'" value='.$draft['num_rows'].'>
											   ...
											   <div class="col-md-8" style="padding-left: 0">'.form_input('txt_quote'.$x.'_'.$p.'', $draft_amount , 'id="txt_quote'.$x.'_'.$p.'" maxlength="17" class="form-control numeric-decimal field-required" style="width: 150px" '.$price_disabled).'</div>
												   ...
												   '.$bom_modals.'
												   '.$btn_bom_viewq.'
												 	'.br(1).'<div>Counter Offer <span id="txt_counteroffer'.$x.'_'.$p.'_text_char_num"> '. ( ((200 - strlen($this->input->post('txt_counteroffer'.$x.'_'.$p.'_text')) == 0)) ? '(You have reached the limit.)': '(' . (200 - strlen($this->input->post('txt_counteroffer'.$x.'_'.$p.'_text'))) . ' characters left)'). '</span>
													</div><br>
												   <div class="col-md-12" id="counter_offer_textarea'.$x.'_'.$p.'" style=" width: 100%;">
													<textarea class="form-control counter_offer_txt_area" id="txt_counteroffer'.$x.'_'.$p.'" name="txt_counteroffer'.$x.'_'.$p.'" maxlength="200" style="height: 100px min-height: 100px max-height: 250px; ">'.$draft['result'][$o]->COUNTER_OFFER.'</textarea>
												   </div>
										...
										<div class="panel panel-primary">
											...
											<div class="row">
												<div class="form-group indent_left">
													...
													<div class="col-md-8 indent_top" id="specs'.$x.'"><a style="cursor:pointer;" onclick="specsview('.$x.');">Specifications <span class="glyphicon glyphicon-modal-window"></span></a> 
											...
											<div class="row">
												<div class="form-group indent_left indent_right" style="padding-left: 20px;">
													<div class="col-md-6">
														<div class="col-md-12 indent_top" id="specifications'.$x.'" style="display: inline;"><textarea class="form-control specs_txt_area" maxlength="3000" id="specs1_text" name="specs1_text" style="min-width: 100%; width: 100%; height: 100px; min-height:100px; max-height: 250px;">'.$row->SPECIFICATION.'</textarea>
														</div>
														<br><br><br>
														<div class="col-md-6">
													...
													<div class="col-md-6">
														...
														<div>
				 										...
				 										<div>Counter Offer <span id="txt_counteroffer'.$x.'_1_text_char_num"> '. ( ((200 - strlen($this->input->post('txt_counteroffer'.$x.'_1_text')) == 0)) ? '(You have reached the limit.)': '(' . (200 - strlen($this->input->post('txt_counteroffer'.$x.'_1_text'))) . ' characters left)'). '</span><br></div>

														<div class="col-md-12" id="counter_offer_textarea'.$x.'_1" style=" width: 100%;">
															<textarea class="form-control counter_offer_txt_area" '.$no_quote_properties.' id="txt_counteroffer'.$x.'_1" name="txt_counteroffer'.$x.'_1" style="height: 100px; min-height:100px; max-height: 250px;" maxlength="200">'.$counter_offer.'</textarea>
														</div>
														...
														<input type="button" name="add_attachmentbtn'.$x.'_1" '.$no_quote_delivery.' id="add_attachmentbtn'.$x.'_1" value="Select Attachment" onclick="attachment_response('.$x.', 1)" style="margin-top: 10px;">
														</div><br>
														...
														<input type="button" class="btn btn-default" id="add_another_quote_btn'.$x.'" value="Add Another Quote" onclick="add_another_bom_quote('.$x.','.(isset($bom_modal_data['line_attachment_id']) ? $bom_modal_data['line_attachment_id'].',1' : '0,0').',&quot;'.$current_currency_data.'&quot;)">
													</div>
		...
		$this->load->view('rfqb/rfq_response_creation_view', $data);
	}
	...
	function add_quotes()
	{
		...
		for ($i=2;$i <= $num_quote; $i++)
		{
			...
			$table .= '<input type="hidden" name="counter_offer_hidden'.$row.'_'.$i.'" id="counter_offer_hidden'.$row.'_'.$i.'" value=0>
					  <div class="form-inline"><div class="col-md-4" style="padding-left: 0">Quote</div>
					  <div class="col-md-8" style="padding-left: 0">'.form_input('txt_quote'.$row.'_'.$i.'', $draft_amount, 'id="txt_quote'.$row.'_'.$i.'" maxlength="17" class="form-control numeric-decimal field-required"  style="width: 150px"').'</div>
					  </div>
					  ...
					  '.br(1).'<div>Counter Offer <span id="txt_counteroffer'.$i.'_text_char_num"> '. ( ((200 - strlen($this->input->post('txt_counteroffer'.$i.'_text')) == 0)) ? '(You have reached the limit.)': '(' . (200 - strlen($this->input->post('txt_counteroffer'.$i.'_text'))) . ' characters left)'). '</span><br></div>

					  <div class="col-md-12" id="counter_offer_textarea'.$row.'_'.$i.'" style="width: 100%;">
					  	<textarea class="form-control counter_offer_txt_area" id="txt_counteroffer'.$row.'_'.$i.'" name="txt_counteroffer'.$row.'_'.$i.'" maxlength="200" style="height: 100px; min-height:100px; max-height:250px;">'.$this->input->post('txt_counteroffer'.$row.'_'.$i).'</textarea>
					  </div>
	   ...
	   $table .= '<input type="hidden" name="counter_offer_hidden'.$row.'_'.$next_row.'" id="counter_offer_hidden'.$row.'_'.$next_row.'" value=0>
				...
				<input type="hidden" name="bom_line_cnt_'.$row.'_'.$next_row.'" id="bom_line_cnt_'.$row.'_'.$next_row.'" value="'.count($bom_modal_data['bom_lines']).'">
				  ...
				  '.br(1).'<div>Counter Offer <span id="txt_counteroffer'.$row.'_'.$next_row.'_text_char_num"> '. ( ((200 - strlen($this->input->post('txt_counteroffer'.$row.'_'.$next_row.'_text')) == 0)) ? '(You have reached the limit.)': '(' . (200 - strlen($this->input->post('txt_counteroffer'.$row.'_'.$next_row.'_text'))) . ' characters left)'). '</span><br></div>

				  <div class="col-md-12" id="counter_offer_textarea'.$row.'_'.$next_row.'" style=" width: 100%;">
				  	<textarea class="form-control" id="txt_counteroffer'.$row.'_'.$next_row.'" name="txt_counteroffer'.$row.'_'.$next_row.'" maxlength="200" style="height:100px; min-height:100px; max-height:250px;">'.$modal_counter_offer.'</textarea>
				  </div>
